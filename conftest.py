...
@pytest.fixture()
def make_subfolder():
    testfilename = ''.join(random.choices(string.ascii_uppercase +
                                          string.digits, k=5))
    subfoldername = ''.join(random.choices(string.ascii_uppercase +
                                           string.digits, k=5))
    if not checkout_positive("cd {}; mkdir {}".format(data["folder_in"],
                                                      subfoldername),
                                                      ""):
        return None, None
...
@pytest.fixture()
def make_badarx():
    checkout_positive("cd {}; 7z a {}/badarx.7z".format(data["folder_in"],
                                                        data["folder_badarx"]),
                                                        "Everything is Ok")
    checkout_positive("truncate -s l {}/badarx.7z".format(data["folder_badarx"]),
                      "Everything is Ok")
    yield "badarx"
...
# Задание 1.
# Условие:
# Дополнить проект фикстурой,
# которая после каждого шага теста дописывает в заранее
# созданный файл stat.txt строку вида:
...
def home_task1():
    ...
    checkout_positive("echo {}>> {}/{}".format(datetime.now().strftime("%H:%M:%S.%f"), data["folder_home"], "stat.txt"),
                      "")
    checkout_positive("echo {} >> {}/{}".format(str(data["count_file"]),
                                                data["folder_home"],
                                                "stat.txt"),
                                                "")
    checkout_positive("echo  {} >> {}/{}".format(data["size_file"],
                                                 data["folder_home"],
                                                 "stat.txt"),
                                                 "")